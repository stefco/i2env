...
RUN mkdir -p ~/.local/share ~/.cache ~/.jupyter \
    ...
    && python -c 'import yaml, pathlib; y=yaml.full_load(pathlib.Path("/root/provision/conda.yml").read_text()); print(" ".join(d["pip"] for d in y["dependencies"] if isinstance(d, dict) and "pip" in d))' \
        | xargs pip install \
